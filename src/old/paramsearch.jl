...
julia> nprocs()
9
julia> @time rs.search_params(rs.rtgM4(); num_sim=100, distributed=true, saveall=true)
  2.378919 seconds (78.95 k allocations: 4.964 MiB)
```
"""
function search_params(m::RTGmodel; num_sim=10, save_iter=num_sim, distributed=false, saveall=true, batch_size=nprocs(), conditions=get_conditions(; df=DataTables.BoolCond), kwags...)
    #create table
    ...
    try
        for _ in 1:length(ran)
            # parameter searching
            res = @showprogress pmap(x -> search_params_once(x[1]; conditions=x[2], x[3]...),
...
"""
function search_params_once(m::RTGmodel; Break=false, idx_hill_coefs=[1], K_dist=K_dist, K_N_dist=K_N_dist, conditions=get_conditions(; df=DataTables.BoolCond), show_valid=true, ssmethod=DEFAULT_SSMETHOD, kwags...)
    # Sampling parameters
    p = init_p(m.model; idx_hill_coefs=idx_hill_coefs, K_dist=K_dist, K_N_dist=K_N_dist)
    m.p[:] = p


    u = get_steadysol(m, m.u; ssmethod=ssmethod).u
